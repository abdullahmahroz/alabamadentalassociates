...
if(!class_exists('Ultimate_Google_Font_Manager'))
{
	class Ultimate_Google_Font_Manager
	{
		function __construct()
		{
			...
			//add_action('admin_menu',array($this, 'google_font_manager_menu'), 100);
		...
		function enqueue_ultimate_google_fonts() {
			...
			wp_register_style('aio-icon-manager',plugins_url('../admin/css/icon-manager.css',__FILE__));
		...
		{
			wp_enqueue_style('aio-icon-manager');
		?>
...
if(!function_exists('enquque_ultimate_google_fonts_optimzed'))
{
	function enquque_ultimate_google_fonts_optimzed($enqueue_fonts)
	{
		...
		if(!empty($enqueue_fonts))
		{
			...
			if($font_count > 0)
				wp_enqueue_style('ultimate-google-fonts', $font_api_call, array(), OPN_VERSION);
		}
	}
}